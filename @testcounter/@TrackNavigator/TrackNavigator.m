...
%             ProgenyID=cell2mat(obj.DisplayTableData(row,3));
    ...
    methods(Static=true)
        ...
        function getPedigreeData(hObject,EventData,hTrackTable)
            ...
            AnnotationTable=struct2table(annotations);
            writetable(AnnotationTable,[pathname '\annotations.txt'],'WriteVariableNames',0);
            ...
            fclose(fid);
            [r,c]=size(pedigreedata);
            condition=repmat(condition,size(pedigreedata,1),1);
            PedigreeData=[condition cell2table(num2cell(pedigreedata)),...
                cell2table(annotationtable)];
            PedigreeData.Properties.VariableNames=heading;
            save([pathname filename(1:end-3) 'mat'],'PedigreeData','-v7.3');
            %write annotation table
            writetable([annotations transitions],[pathname filename(1:end-4),' annotations' '.txt'],'WriteVariableNames',1);
            AnnotationsTable=[annotations transitions];
            save([pathname 'statetransitions.mat'],'AnnotationsTable','-v7.3');
...
%                 disp(['Processing track ' num2str(i)]);
        ...
        function getTrajectories(hObject,EventData,hTrackTable)
            ...
            fclose(fid);
            [r,c]=size(distancetable);
            DistanceTbl=cell2table(num2cell(distancetable));  
            DistanceTbl.Properties.VariableNames=heading;
            save([pathname filename(1:end-3) 'mat'],'DistanceTbl','-v7.3');
...
%                 disp(['Processing track ' num2str(i)]);
            ...
            if length(varargin)==3
                ...
                save([pathname filename(1:end-3) 'mat'],'FinalTable','-v7.3');
                close(h);
            elseif length(varargin)==1
                return
            end
