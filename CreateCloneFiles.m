function complete_clone=CreateCloneFiles(tracktable,tbl,TimeStamps,varargin)
tracks=tracktable.TableData;
pedigreedata=tracktable.PedigreeData;
    ...
    for j=1:length(trackids)
        ...
        if ~isempty(pedigreedata)
            if isfield(pedigreedata{i}.track{j},'CropCoordinates')
                complete_clone{i}.track{j}.CropCoordinates=pedigreedata{i}.track{j}.CropCoordinates;
            end
        end
