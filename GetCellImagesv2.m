...
function clone=GetCellImagesv2(clone,clonenumb,tracknumbers,buff,path,channels,imagestack)
    ...
    for h=1:length(clonenumb)
        ...
        for i=1:length(tracknumb)
            ...
            for j=1:1:length(imagetimes)
                ...
                [cellimage,relframeid,cmap,crop_coordinates]=GetCellImagePatchesv2(clone,currentclone,tracknumb(i),time,buff,channels{g},imageobj);
                if j==1
                    [r,c]=size(cellimage);
