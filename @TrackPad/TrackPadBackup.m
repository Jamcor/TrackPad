...
%                 frametextwidth=get(hTrackPad.FrameTextBox,'Extent');
%                 hTrackPad.FrameTextBox.Position=[0,pos(4)/40,fextent(3),fextent(4)];
                hTrackPad.FrameTimeBox.String=['Timestamp ', num2str(h),':',...
                    num2str(m),':',num2str(s),' (H:M:S)'];
                hTrackPad.FrameTimeBox.Units='Normalized';
                hTrackPad.FrameTextBox.Units='Normalized';
%                 hTrackPad.FrameTimeBox.Position=[fextent(3)*1.05,pos(4)/40,hTrackPad.FrameTimeBox.Extent(3),hTrackPad.FrameTimeBox.Extent(4)];
%                 hTrackPad.FrameTimeBox.Position(3)=hTrackPad.FrameTimeBox.Extent(3);
                
                hTrackPad.CursorPositionBox.Units='Normalized';
%                 x=hTrackPad.FrameTimeBox.Extent(3) + hTrackPad.FrameTextBox.Extent(3);
%                 hTrackPad.CursorPositionBox.Position=[1-hTrackPad.CursorPositionBox.Extent(3)*1.1,pos(4)/40,hTrackPad.CursorPositionBox.Extent(3),hTrackPad.CursorPositionBox.Extent(4)];
                ...
                for i=1:m
                    ...
                    if ~isempty(Track{n}) && sum(n~=range)==2 %update annotations for all but first frame
                        if isempty(Track{n}.AnnotationHandle)
                            ...
                            x=Track{n}.Position(1,1)+Track{n}.Position(1,3)/2;
                            y=Track{n}.Position(1,2)+Track{n}.Position(1,4)/2;
                            Track{n}.AnnotationHandle.Position(1)=x; 
                            Track{n}.AnnotationHandle.Position(2)=y;
        ...
        function PlayBackward(varargin)
            ...
            hTrackPad.ImageStack.CurrentNdx=1;
            hTrackPad.ImageStack.LastNdx=hTrackPad.ImageStack.CurrentNdx+1;
        end
